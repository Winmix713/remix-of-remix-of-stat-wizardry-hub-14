        ...
        <ul className="md:hidden divide-y divide-white/5">
          {matches.map((match, index) => (
            <li key={index} className="p-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-1">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`size-2 shrink-0 rounded-full ${teamColors[match.home_team] || 'bg-gray-500'}`} />
                    <span className="truncate text-sm">{match.home_team}</span>
                  </div>
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`size-2 shrink-0 rounded-full ${teamColors[match.away_team] || 'bg-gray-500'}`} />
                    <span className="truncate text-sm">{match.away_team}</span>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs ring-1 ${match.btts_computed ? "bg-sports-emerald/15 text-sports-emerald ring-sports-emerald/20" : "bg-white/10 text-white/80 ring-white/15"}`}>
                  BTTS: {match.btts_computed ? "Igen" : "Nem"}
                </span>
                <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs ring-1 ${match.comeback_computed ? "bg-primary/15 text-primary ring-primary/20" : "bg-white/10 text-white/80 ring-white/15"}`}>
                  Fordítás: {match.comeback_computed ? "Igen" : "Nem"}
                </span>
                <span className="text-xs text-white/50 ml-auto">{match.match_time}</span>
              </div>
              <Button
                variant="outline"
                size="sm"
